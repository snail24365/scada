...
const MiniMap = ({ width }: Props) => {
  ...
  function drawMinimap() {
    ...
    shrinkedRootSvg.setAttribute(
      "viewBox",
      `0 0 ${viewport.resolutionX} ${viewport.resolutionY}`
    );
  ...
  function updateZoom() {
    const isZoomed =
      viewport.resolutionX !== viewbox.width ||
      viewport.resolutionY !== viewbox.height;
  ...
  function calculateNextViewboxXY(
    ...
    const nextXY = new Vector2(e.clientX, e.clientY)
      ...
      .clamp(
        new Vector2(0, 0),
        new Vector2(width - zoomBox.width, height - zoomBox.height)
      )
